...
void order(struct node *insert)
{
    ...
    if(strcmp(insert->lname, head->lname) == 0) //same last name as head
    {
        if(strcmp(insert->fname, p->fname) < 0)
        {
            insert->next = head;
            head = insert;
            return;
    ...
    else if(strcmp(insert->lname, head->lname) < 0) // smaller last name to head
    {
        insert->next = head;
        head = insert;
        return;
    ...
    else //greater than head   
    {
        while(p != NULL) 
        {
            if(strcmp(insert->lname, p->lname) == 0)
            {
                if(strcmp(insert->fname, p->fname) < 0)
                {
            ...
            else if(strcmp(insert->lname, p->lname) < 0)
            {
                insert->next = before->next;
                before->next = insert;
                return;
